...
export class DrawManager {
    ...
    /* */
    drawArrow(origin, orientation, length, color = '#000000') {
        ...
        this.context.stroke();
        this.context.beginPath();
        const canvasFirstBound = this.transformToCanvasCoord(Vec2.fromAngle(orientation).mult(-length / ARROW_END_LENGTH_FACTOR).rotate(ARROW_END_POINT_ANGLE).add(endPoint));
        this.context.moveTo(canvasEndPoint.x, canvasEndPoint.y);
        this.context.lineTo(canvasFirstBound.x, canvasFirstBound.y);
        const canvasSecondBound = this.transformToCanvasCoord(Vec2.fromAngle(orientation).mult(-length / ARROW_END_LENGTH_FACTOR).rotate(-ARROW_END_POINT_ANGLE).add(endPoint));
        this.context.moveTo(canvasEndPoint.x, canvasEndPoint.y);
